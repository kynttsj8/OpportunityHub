...
export const bookmarkOpportunity = async (req, res) => {
    try {
        ...
        // Check if the opportunity is already bookmarked by the user
        if (user.bookmarks.includes(opportunityId)) {
            return res.status(400).json({ message: "Opportunity already bookmarked", success: false });
        }

        user.bookmarks.push(opportunityId);
        await user.save();

        return res.status(200).json({ message: "Opportunity bookmarked successfully", success: true });
    } catch (error) {
        console.log(error);
